...
#	_save_stats()
	_load_stats()
	sp = (Lvl/5) - _check_enabled_skills()
	_set_sp()
	_enable_true_skills()
	_check_upgrades()
...
func _load_points():
	var load_file = File.new()
	var LOAD_PATH = "user://statFile.json"
	
	if not load_file.file_exists(LOAD_PATH):
		print("file does not exist")
		return
	
	var err = load_file.open_encrypted_with_pass(LOAD_PATH, load_file.READ, "mypass")
	var current_line = {}
	current_line = parse_json(load_file.get_line())
	Lvl = int(current_line["Lvl"])
